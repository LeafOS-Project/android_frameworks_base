...
public final class AttestationHooks {
    ...
    public static void patchBuildFp(String type) {
        Log.d(TAG, "Get service - algorithm=" + type);
        if (sIsGms && "KeyStore".equals(type)) {
            String originalFp = Build.FINGERPRINT;
            Log.d(TAG, "Patch build for KeyStore " + originalFp + " -> " + FAKE_FINGERPRINT);
            ...
            Thread thread = new Thread(() -> {
                try {
                    ...
                    Log.d(TAG,
                            "Unpatch build for KeyStore " + FAKE_FINGERPRINT + " -> " + originalFp);
